...
class HomeController < ApplicationController
  ...
  # skip_before_action :authenticate!, :only => [:login, :get_code, :get_email], :raise=>false
  require 'json'
  ...
  def calendar_action
    ...
    case mode
      when "inserted"
        ...
        @refresh_token
        @oauth_expirates_at
        email = attendees[:email][0]
        User.all.each do |u|
          if u.email.to_s == email.to_s
            @token = u.oauth_token
            @refresh_token = u.oauth_refresh_token
            @oauth_expirates_at = u.oauth_expirates_at
          end
        end
        ...
        create[:data] ||= []
        create_event = {}
        create_event[:accounts] ||= []
        account = {}
        account[:email] ||= {}
        account[:token] ||= {}
        account[:token][:acces_token] ||= {}
        account[:token][:refresh_token] ||= {}
        account[:token][:expiration_time_millis] ||= {}
        account[:email] ||= attendees[:email][0]
        account[:token][:acces_token] ||= @token
        account[:token][:refresh_token] ||= @refresh_token
        account[:token][:expiration_time_millis] ||= @oauth_expirates_at
        create_event[:accounts] << account.clone
        create_event[:period] ||= {}
        create_event[:period][:start] ||= {}
        create_event[:period][:end] ||= {}
        create_event[:period][:name] ||= {}
        create_event[:period][:start] ||= event[:start]
        create_event[:period][:end] ||= event[:end]
        create_event[:period][:name] ||= event[:summary]

        create[:data] << create_event.clone
        ...
        sendPOST('http://ppl-scheduling.herokuapp.com/createevent', JSON.dump(create))
    end
